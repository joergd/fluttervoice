module UserSystem
  ...
  def admin_required
    ...
    return session[:user].email == "joergd@pobox.com"
  end
  ...
  def store_location(url=nil)
     session[:return_to] = url || request.request_uri # + s
  end
  ...
  def primary_user?
    if !@account.nil?
      user? && @account.primary_person_id == session[:user].id
    else
      false
    end
  end

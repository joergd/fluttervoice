...
module ActiveMerchant
  module Billing
    ...
    class CreditCard
      ...
      # required
      attr_accessor :number, :month, :year, :type, :first_name, :last_name, :name
      ...
      def validate   
        ...
        @errors.add "name", "cannot be empty"                     unless name
      ...
      # optional 
      attr_accessor :company, :address1, :address2, :city, :state, :zip, :country, :phone, :fax, :email, :addrnum
      attr_accessor :cvmvalue, :cvmindicator, :track, :verification_number
      
      def name
        name? && !@name.blank? ? @name : "#{@first_name} #{@last_name}" 
      end

  ...
  module Billing
    ...
    class AuthorizedNetGateway < Base
      ...
      def capture(money, identification, options = {})
        commit('PRIOR_AUTH_CAPTURE', money, options.merge(:trans_id => identification))
      end       
      ...
      def add_creditcard(post, creditcard)      
        ...
        post[:first_name] = creditcard.first_name      if creditcard.first_name?
        post[:last_name]  = creditcard.last_name       if creditcard.last_name?
        post[:address]    = creditcard.address1        if creditcard.address1?
        post[:city]       = creditcard.city            if creditcard.city?
        post[:state]      = creditcard.state.upcase    if creditcard.state?
        post[:country]    = creditcard.country.upcase  if creditcard.country?
        post[:zip]        = creditcard.zip             if creditcard.zip?
      end

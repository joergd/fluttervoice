...
module ActiveMerchant
  module Billing
    ...
    class TrustCommerceGateway < Base
      ...
      def add_creditcard(parameters, creditcard)
        parameters[:media]     = "cc"
        parameters[:cc]        = creditcard.number      
        parameters[:exp]       = expdate(creditcard)
        parameters[:name]      = creditcard.name if creditcard.name?
        parameters[:address1]  = creditcard.address1 if creditcard.address1?
        parameters[:address2]  = creditcard.address2 if creditcard.address2?
        parameters[:city]      = creditcard.city if creditcard.city?
        parameters[:state]     = creditcard.state if creditcard.state?
        parameters[:zip]       = creditcard.zip if creditcard.zip?
        parameters[:country]   = creditcard.country if creditcard.country?  
      end
